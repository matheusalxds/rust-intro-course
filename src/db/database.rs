...
use dotenv::dotenv;
use mysql_async::{Error, params, prelude::Queryable, Value};

use crate::models::{
	incoming::{GetProductLocationsByCode, GetProductLocationsByName},
	outgoing::RemoveUniqueIdentifierRequest,
};
// ----------  TEST 1, TEST 2 -------- //
use crate::models::incoming::{AddOrUpdateUniqueIdentifierRequest, UniqueIdentifier};
...
impl Database {
	...
	pub async fn get_product_locations_by_name(
		&self,
		product: &GetProductLocationsByName,
	) -> Result<Vec<UniqueIdentifier>, Error> {
		let product_name = &product.product_name;
		let query = "SELECT * FROM unique_identifiers WHERE product_name = :product_name";
		let named_params = params! { "product_name" => product_name };
	...
	pub async fn get_product_locations_by_code(
		&self,
		product: &GetProductLocationsByCode,
	) -> Result<Vec<UniqueIdentifier>, Error> {
	...
	pub async fn add_or_update_unique_identifier(
		&self,
		update_data: &AddOrUpdateUniqueIdentifierRequest,
		...
		conn.exec_drop(query_unique_identifier, params_unique_identifier)
			.await?;
	...
	pub async fn remove_unique_identifier(
		&self,
		update_data: &RemoveUniqueIdentifierRequest,
	) -> Result<(), mysql_async::Error> {
		...
		let current_pcs: Option<i32> = self
			.pool
			.get_conn()
			.await?
			.exec_first(check_current_pcs_query, check_current_pcs_params)
			.await?;
		...
		if let Some(current_pcs_value) = current_pcs {
			if update_data.pcs > current_pcs_value {
				...
				return Err(mysql_async::Error::from(std::io::Error::new(
					std::io::ErrorKind::InvalidInput,
					"Not enough pcs for deduction",
				)));
			}
		} else {
		...
		let check_updated_pcs_query =
			"SELECT pcs FROM unique_identifiers WHERE concatenated_string = ?";
		let check_updated_pcs_params: Vec<Value> = vec![Value::from(&update_concatenated_string)];
		let updated_pcs: Option<i32> = conn
			.exec_first(check_updated_pcs_query, check_updated_pcs_params)
			.await?;
...
mod tests {
			...
			Ok(locations) => {
				if locations.is_empty() {
					...
					for location in locations {
						assert!(
							!location.product_code.is_empty(),
							"Product code should not be empty"
						);
						assert!(
							!location.color.is_empty(),
							"Product code is missing or empty"
						);
						assert!(
							!location.product_name.is_empty(),
							"Product code is missing or empty"
						);
						assert!(
							!location.warehouse.is_empty(),
							"Product code is missing or empty"
						);
						assert!(
		...
		match result {
			Ok(unique_identifiers) => {
				...
				for product in unique_identifiers {
					assert!(
						!product.product_code.is_empty(),
						"Product code is missing or empty"
					);
					assert!(
						!product.color.is_empty(),
						"Product code is missing or empty"
					);
					assert!(
						!product.product_name.is_empty(),
						"Product code is missing or empty"
					);
					assert!(
						!product.warehouse.is_empty(),
						"Product code is missing or empty"
					);
					assert!(
						!product.location.is_empty(),
						"Product code is missing or empty"
					);
	...
	async fn get_product_locations_by_code() {
		...
		let db = setup_test_database().await;
		// Act: Call the function you want to test
		let product_code = "806807071422".to_string();
	...
	async fn test_add_or_update_unique_identifier() {
		...
		let db = setup_test_database().await;
		// Act: Call the function you want to test
		let update_data: AddOrUpdateUniqueIdentifierRequest = AddOrUpdateUniqueIdentifierRequest {
		...
		// Assert: Check if the result is as expected
		assert!(result.is_ok(), "Function should execute without errors3")
	}
	...
	async fn test_remove_unique_identifier() {
		...
		let db = setup_test_database().await;
		// Act: Call the function you want to test
		let update_data: RemoveUniqueIdentifierRequest = RemoveUniqueIdentifierRequest {
			color: "Aquamarine".to_string(),
			product_name: "Aromatherapy diffuser".to_string(),
			warehouse: "HALA 5".to_string(),
			location: "M5-A-1".to_string(),
